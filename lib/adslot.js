...
var AdSlot = exports.AdSlot = function (_React$Component) {
  ...
  function AdSlot() {
    ...
    }, _this.doRegisterSlot = async function () {
      await _manager2.default.registerSlot(_extends({}, (0, _utils.mapContextToAdSlotProps)(_this.context), _this.props, _this.state, {
        slotShouldRefresh: _this.slotShouldRefresh
      }));

      if (_this.props.fetchNow) {
        await _manager2.default.load(_this.getSlotId());
      }

      _manager2.default.attachSlotRenderEnded(_this.slotRenderEnded);
    }, _this.registerSlot = async function () {
      if (_this.state.slotId === null) {
        _this.setState({ slotId: _this.generateSlotId() }, _this.doRegisterSlot);
      } else {
        await _this.doRegisterSlot();
      }
  ...
  _createClass(AdSlot, [{
    ...
    value: async function componentWillReceiveProps(nextProps) {
