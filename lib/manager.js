...
var DFPManager = Object.assign(new _events.EventEmitter().setMaxListeners(0), {
  ...
  refresh: function refresh() {
    ...
    var slotsToRefresh = function slotsToRefresh(slots) {
      var refreshableSlots = _this4.getRefreshableSlots();

      if (slots.length === 0) {
        return Object.keys(refreshableSlots).map(function (slotId) {
          return refreshableSlots[slotId].gptSlot;
        });
      }
    ...
    if (loadAlreadyCalled === false) {
      ...
      this.getGoogletag().then(function (googletag) {
        googletag.cmd.push(function () {
          ...
          pubadsService.refresh(slotsToRefresh(slots));
        });
      });
    }
  },
  destroyGPTSlots: function destroyGPTSlots() {
    for (var _len2 = arguments.length, slotsToDestroy = new Array(_len2), _key2 = 0; _key2 < _len2; _key2++) {
      slotsToDestroy[_key2] = arguments[_key2];
    }
