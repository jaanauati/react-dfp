...
var DFPManager = Object.assign(new _events.EventEmitter().setMaxListeners(0), {
  ...
  getGoogletag: function getGoogletag() {
    ...
    if (googleGPTScriptLoadPromise === null) {
      googleGPTScriptLoadPromise = Utils.loadGPTScript()["catch"](function (error) {
        _this2.emit('gptScriptLoadError', {
          error: error
        });
      });
    }
  ...
  load: function load() {
    ...
    if (loadPromise === null) {
      ...
      loadPromise = loadPromise.then(function () {
        return _this3.doLoad.apply(_this3, slots);
      });
    }
  },
  ...
  gptLoadAds: function gptLoadAds(slotsToInitialize) {
    ...
    return new Promise(function (resolve) {
      _this4.getGoogletag().then(function (googletag) {
        _this4.configureInitialOptions(googletag);
        ...
        slotsToInitialize.forEach(function (currentSlotId) {
          ...
          googletag.cmd.push(function () {
            ...
            if (gptSlot !== null) {
              ...
              var slotTargetingArguments = _this4.getSlotTargetingArguments(currentSlotId);
              ...
              var slotAdSenseAttributes = _this4.getSlotAdSenseAttributes(currentSlotId);
        ...
        _this4.configureOptions(googletag);
  ...
  configureInitialOptions: function configureInitialOptions(googletag) {
    ...
    googletag.cmd.push(function () {
      if (_this5.disableInitialLoadIsEnabled()) {
  ...
  configureOptions: function configureOptions(googletag) {
    ...
    googletag.cmd.push(function () {
      var pubadsService = googletag.pubads();
      pubadsService.setRequestNonPersonalizedAds(_this6.personalizedAdsEnabled() ? 0 : 1);

      var targetingArguments = _this6.getTargetingArguments(); // set global targetting arguments
      ...
      var adSenseAttributes = _this6.getAdSenseAttributes();

      Object.keys(adSenseAttributes).forEach(function (key) {
        pubadsService.set(key, adSenseAttributes[key]);
      });

      if (_this6.lazyLoadIsEnabled()) {
        var args = [];

        var config = _this6.getLazyLoadConfig();
      ...
      if (_this6.singleRequestIsEnabled()) {
        pubadsService.enableSingleRequest();
      }

      if (_this6.collapseEmptyDivs === true || _this6.collapseEmptyDivs === false) {
        pubadsService.collapseEmptyDivs(_this6.collapseEmptyDivs);
      }
    });
  },
  ...
  attachGPTLoadError: function attachGPTLoadError(cb) {
