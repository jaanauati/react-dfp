...
var DfpManager = function DfpManager() {
    ...
    if (_this.isManagerInitialized === false) {
      ...
      googletag.cmd.push(function () {
        ...
        pubAds.addEventListener('slotRenderEnded', function (ev) {
          return _this._eventEmitter.emit('slotRenderEnded', { slotId: ev.slot.getSlotElementId(), event: ev });
        });

        Object.keys(_this.targetingArguments).forEach(function (keys) {
          return pubAds.setTargeting(key, _this.targetingArguments[key]);
        });
      });
    }
  ...
  this.load = async function (slotId) {
    ...
    googleTag.cmd.push(function () {
      ...
      if (_this.collapseEmptyDivs === true || _this.collapseEmptyDivs === false) {
  ...
  this.attachSlotRenderEnded = function (callback) {
    return _this._eventEmitter.on('slotRenderEnded', callback);
  };

  this.detachSlotRenderEnded = function (callback) {
    return _this._eventEmitter.removeListener('slotRenderEnded', callback);
  };
