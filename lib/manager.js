...
var DFPManager = _extends(new _events.EventEmitter().setMaxListeners(0), {
  ...
  destroySlots: function destroySlots(slotsToDestroy) {
    if (managerAlreadyInitialized === true) {
      loadAlreadyCalled = false;
      this.getGoogletag().then(function (googletag) {
        if(slotsToDestroy !== undefined) {
          googletag.destroySlots(Object.keys(slotsToDestroy).map(function (slotId) {
            return slotsToDestroy[slotId].gptSlot;
          }));
        } else {
  ...
  setNewAdUnit: function setNewAdUnit(slots, newAdUnit) {
    if (managerAlreadyInitialized === true) {
      Object.keys(slots).forEach(function (currentSlotId) {
